...
public class LayerRoot extends RelativeLayout implements ILayerHost {
    ...
    public void addLayer(final ILayer layer) {
        ...
        if (mLayerMap.get(layer.getZIndex()) != null) {
            Log.d(TAG, "layerType:" + layer.getZIndex()
                    + " already exist, remove the old before adding new one! "
                    + hashCode());
            return;
        }

        Log.d(TAG, "add layer:" + layer.getClass().getSimpleName() + " layerType:" + layer.getZIndex() + " " + hashCode());
        ...
        Pair<View, LayoutParams> pair = layer.onCreateView(getContext(), LayoutInflater.from(getContext()));
    ...
    public void removeLayer(final ILayer layer) {
        ...
        Log.d(TAG, "removeLayer:" + layer.getClass().getSimpleName()
