...
public class FeedShareActivity extends SolutionBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initUI();
        ViewCompat.setOnApplyWindowInsetsListener(mTitleBar, (v, windowInsets) -> {
        ...
        rtmClient.joinRoom(new IRequestCallback<JoinRoomResponse>() {
            ...
            public void onError(int errorCode, String message) {
                if (RTSBaseClient.ERROR_CODE_USERNAME_SAME == errorCode) {
                    SolutionToast.show(FeedShareActivity.this, "该用户已存在房间中", Toast.LENGTH_SHORT);
                    finish();
                    return;
                }
                if (RTSBaseClient.ERROR_CODE_ROOM_FULL == errorCode) {
                    SolutionToast.show(FeedShareActivity.this, "房间人数已满", Toast.LENGTH_SHORT);
                    finish();
                    return;
                }
                String msg = "joinRoom biz failed message: " + message + ",errorCode:" + errorCode;
                SolutionToast.show(FeedShareActivity.this, msg, Toast.LENGTH_SHORT);
                Log.i(TAG, msg);
            }
        });
    }
    ...
    private void attachRenderView(FrameLayout parentView, String uid, boolean isRemoteUser) {
        ...
        if (isRemoteUser) {
            RemoteStreamKey streamKey = new RemoteStreamKey(FeedShareDataManger.getInstance().getRoomId(),
                    uid, StreamIndex.STREAM_INDEX_MAIN);

            engine.setRemoteVideoCanvas(streamKey, new VideoCanvas());
            engine.setRemoteVideoCanvas(streamKey, videoCanvas);
        } else {
            engine.setLocalVideoCanvas(StreamIndex.STREAM_INDEX_MAIN, new VideoCanvas());
    ...
    public void onReconnectToRoom(SDKReconnectToRoomEvent event) {
        ...
        rtsClient.reconnectToServer(event.roomId, new IRequestCallback<RTSBizResponse>() {
            @Override
    ...
    public void onFinishRoom(FinishRoomInform finishRoomEvent) {
        ...
        if (TextUtils.equals(finishRoomEvent.roomId, FeedShareDataManger.getInstance().getRoomId())) {
            final boolean isHost = FeedShareDataManger.getInstance().isHost();
            if (finishRoomEvent.type == FinishRoomInform.TYPE_TIMEOUT) {
                if (isHost) {
                    SolutionToast.show(this, R.string.duration_live_has_reached_minutes, Toast.LENGTH_SHORT);
                } else {
                    SolutionToast.show(this, "房主已关闭直播", Toast.LENGTH_SHORT);
                }
            } else if (!isHost) {
                SolutionToast.show(this, "房主已关闭直播", Toast.LENGTH_SHORT);
            }
