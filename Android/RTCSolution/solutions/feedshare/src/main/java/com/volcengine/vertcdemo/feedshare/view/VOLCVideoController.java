...
public class VOLCVideoController implements VideoController, VideoInfoListener {
    ...
    private final VideoEngineCallback mVideoEngineCallback = new VideoEngineCallback() {
        @Override
        ...
        public void onBufferStart(final int reason, final int afterFirstFrame, final int action) {
            Log.d(TAG, "onBufferStart reason " + reason
        ...
        public void onBufferEnd(final int code) {
            Log.d(TAG, "onBufferEnd code " + code);
            for (VideoPlayListener listener : playListeners) {
                if (listener != null) {
        ...
        public void onStreamChanged(TTVideoEngine engine, int type) {
            Log.d(TAG, "onStreamChanged type " + type);
            for (VideoPlayListener listener : playListeners) {
                if (listener != null) {
    ...
    private void initEngine() {
        if (mVideoEngine == null) {
            ...
            // use video hardware
            mVideoEngine.setIntOption(TTVideoEngine.PLAYER_OPTION_ENABLE_HARDWARE_DECODE,
            ...
            if (mAudioProcessor != null) {
                mVideoEngine.setObjectOption(TTVideoEngine.PLAYER_OPTION_AUDIO_PROCESSOR_OBJECT, mAudioProcessor);
                //控制播放器不进行音频渲染
